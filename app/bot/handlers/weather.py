...
async def cmd_today(message: Message, user: dict, lang: str) -> None:
    ...
    if data is None:
        ...
        return
    await message.answer(format_today(data, user["place_name"], lang), parse_mode="Markdown")


@router.message(Command("hafta"))
@router.message(MenuTextFilter("menu_week"))
async def cmd_week(message: Message, user: dict, lang: str) -> None:
    if not await _has_location(message, user, lang):
        return
    data = await fetch_forecast(user["lat"], user["lon"])
    if data is None:
        await message.answer(i18n.t("error_api_down_no_cache", lang))
        return
    await message.answer(format_week(data, user["place_name"], lang), parse_mode="Markdown")
